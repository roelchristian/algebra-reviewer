...
we have established thus far). Let \(F(X)\) be the set of all reduced words in
\(X\). Then the empty word \(\epsilon\) is a reduced word and must be in
\(F(X)\). Define a binary operation \(\cdot\) on \(F(X)\) by
\[
    w_1 \cdot w_2 = \overline{w_1 w_2}
\]
where \(w_1, w_2 \in F(X)\) and \(w_1 w_2\) is the concatenation of \(w_1\) and
\(w_2\). We claim that \((F(X), \cdot)\) is a group under this operation.
...
operation \(\cdot\) is associative. Let \(w_1, w_2, w_3\) be words in \(F(X)\).
...
\begin{proof}
    ...
    a reduced word and \(x \in X\), then \(\Phi(x\cdot w) = \phi(x)\Phi(w)\) and
    \(\Phi(x^{-1} \cdot w) = \phi(x)^{-1}\Phi(w)\). From this it would then
    follow, using the fact that the operation \(\cdot\) on \(F(X)\) is
    associative, and by induction on the length of the reduced word \(w\), that
    for all reduced words \(w_1\) and \(w_2\), we have \(\Phi(w_1 \cdot w_2) =
    ...
    \end{align*}
    If \(x = x_1\) and \(\epsilon_1 = -1\), then the reduction \(x \cdot w\) is
    the reduced word \(x_2^{\epsilon_2} \cdots x_r^{\epsilon_r}\), so that
    \begin{align*}
    ...
    \(\phi\). This completes the proof that \(F(X)\) is the free group on \(X\).
\end{proof}

\begin{example}
    Consider the set \(X = \{x\}\). We claim that \(F(X) \cong \Z\). We shall
    show this first by using the universal property of free groups and again by
    an explicit construction of \(F(X)\).

    % Proof by universal property
    By the universal property for free groups, if \(F(X) \cong \Z\), there
    exists a function \(\iota : X \to \Z\) such that for any group \(G\) and any
    set function \(\phi: X \to G\), there exists a unique group homomorphism
    \(\Phi: \Z \to G\) such that \(\Phi \circ \iota = \phi\). Consider the
    function \(\iota : X \to \Z\) that sends \(x \in X\) to some element \(n \in
    \Z\). Since \(\Phi\) must be a group homomorphism, \(n\)

    % Proof by explicit construction
    On the other hand, we can construct \(F(X)\) explicitly. The set of reduced
    words in \(X\) is the set of all words in \(x\) that do not contain any
    substring of the form \(xx^{-1}\) or \(x^{-1}x\). This is the set of all
    words of the form
    \[
        x_1 x_2 \cdots x_n,
    \]
    where each \(x_i = x \in X\). Writing \(x^n = x x \cdots x\) for \(n\)
    copies of \(x\), we can see that the set of reduced words in \(F(X)\) is the
    cyclic group \(\langle x \rangle\), which is isomorphic to \(\Z\).

...
\begin{example}[Units, division rings, fields]
    ...
    of a unit is itself a unit. Left (resp. right) units are always non zero
    divisors: indeed, if \(u\) is a left (resp. right) unit in a ring \(R\),
    then there exists an element \(v \in R\) such that \(uv = 1\) (resp. \(vu =
    1\)). If \(u\) is a right (resp. left) zero divisor, then there exists a
    nonzero element \(a \in R\) such that \(au = 0\) (resp. \(ua = 0\)). But
    then (in the case of a right zero divisor) we have
    \[
        a = a(1) = a(uv) = (au)v = 0v = 0
    \]
    and thus \(a = 0\), a contradiction. Thus left (resp. right) units are never
    zero divisors.
    ...
    then \(R\) is a field if and only if \(R\) is an integral domain.
\end{example}

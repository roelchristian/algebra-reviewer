...
\begin{example}[Symmetric groups]
    ...
    \ldots, n\}\), then we write \(S_n\) instead of \(S_X\). Observe that for
    any set \(X\), \(S_X\) is exactly the set of all automorphisms \(\Aut(X)\)
    in the category \(\Set\) we have defined in
...
\begin{example}[Dihedral groups]
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            % node mapsto
            \node at (3, 0.866) {\(\overset{\sigma_A}{\mapsto}\)};
        \end{tikzpicture}
        \begin{tikzpicture}
            \coordinate (A) at (0, 0); \coordinate (B) at (2, 0); \coordinate
            (C) at (1, 1.732); \draw (A) -- (B) -- (C) -- cycle;
            ...
            % node mapsto
            \node at (3, 0.866) {\(\overset{\rho_1}{\mapsto}\)};
        \end{tikzpicture}
        \begin{tikzpicture}
            \coordinate (A) at (0, 0); \coordinate (B) at (2, 0); \coordinate
            (C) at (1, 1.732); \draw (A) -- (B) -- (C) -- cycle;
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            % node mapsto
            \node at (3, 0.866) {\(\overset{\sigma_C}{\mapsto}\)};
        \end{tikzpicture}
        \begin{tikzpicture}
            \coordinate (A) at (0, 0); \coordinate (B) at (2, 0); \coordinate
            (C) at (1, 1.732); \draw (A) -- (B) -- (C) -- cycle;
    ...
    identities:
    \begin{align*}
        \rho_2 &= \rho_1 \circ \rho_1\\
        \sigma_B &= \rho_1 \circ \rho_1 \circ \sigma_A\\
        \sigma_C &= \rho_1 \circ \sigma_A.
    \end{align*}
    Writing \(\rho\) for \(\rho_1\) and \(\sigma\) for \(\sigma_A\) and using
    the exponential notation we have introduced in
    \S~\ref{sec:definition-and-basic-properties-grp}, the distinct elements of
    \(D_3\) are then
    \[
        R = \{e = \rho_0, \rho, \rho^2, \sigma, \sigma\rho, \sigma\rho^2\}.
    \]

    With our notation fixed, we observe that \(e \circ \tau = \tau \circ e =
    \tau\) for all maps \(\tau \in R\). Moreover, any composition of elements of
    \(R\) is again an element of \(R\) and this composition is likewise
    associative. Similarly, each element of \(R\) has an inverse in \(R\) (the
    inverse of a rotation is the rotation in the opposite direction, and the
    inverse of a reflection is the reflection about the same axis of symmetry).
    Thus \(R\) is a group under composition of functions. We usually denote this
    group by \(D_3\), the \emph{dihedral group} of order \(6\) (i.e., the group
    of symmetries of a regular triangle). 
...
Consider the map \(\phi : D_3 \to S_3\). An informal way to look at this is as
follows: if we fix the order of the vertices of an equilateral triangle \(ABC\)
(for example, let \(A\) be the `first' vertex, \(B\) the second and \(C\) the
last) then the rotations \(e, \rho, \rho^2\) can be identified with a
...
\(S_3\). A formal check shows that this `identification' is an isomorphism of
groups and thus \(D_3 \cong S_3\). On the other hand \(D_4\) has order \(8\)
while \(S_4\) has order \(24\), and thus \(D_4 \not\cong S_4\). However using
the same method we can show that \(D_4 \hookrightarrow S_4\). The question then
is: can we do extend for arbitrary groups \(G\)? That is, can we characterize
any group \(G\) as permutations of some set \(X\)? We explore this train of
thought in this section, culminating in a result known as Cayley's theorem that
provides an answer to this question and some other interesting results.
...
group \(S_X\) such that \(\alpha_e = \id_X\) and \(\alpha_{gh} = \alpha_g \circ
\alpha_h\) for all \(g, h \in G\) (because \(\alpha\) is a group homomorphism).
Since the image \(\alpha_g(x)\) is then an element of \(X\), \(\alpha\) can then
be thought of as a map which takes an element \(g \in G\) and an element \(x \in
X\) and returns another element \(\alpha(g, x) \in X\) that is the image of
\(x\) under the permutation \(\alpha_g\). Thus we can give the following
definition in the context of \(\Set\).
...
\begin{example}
    ...
    \[
        \gamma(g, x) = gxg\inv
    \]
    is an action of \(G\) on itself. Indeed, we have \(\gamma(e, x) = exe\inv =
    x\) and
    \[

...
\begin{example}
    ...
    Suppose \(C_2 \oplus C_3\) is a coproduct in \(\Grp\); then there would exist a unique group homomorphism \(\Phi: C_2 \oplus C_3 \to S_3\) such that the following diagram commutes:
    \[
        \begin{tikzcd}
            & S_3 & \\
    ...
    \end{align*}
    But since \(S_3\) is nonabelian, this is not generally true. Therefore \(C_2 \oplus C_3\) is not a coproduct in \(\Grp\).
\end{example}

...
\begin{example}[Units, division rings, fields]
    ...
    map \(R \to R\). We claim that an element \(u \in R\) is a unit if and only
    if the left (resp. right) multiplication map \(x \mapsto ux\) (resp. \(x
    \mapsto xu\)) is surjective. We establish the claim for right
    multiplication, with the left case being analogous. Write \(\rho_u : R \to
    R\) for the right multiplication map \(x \mapsto xu\). If \(u\) is a right
    unit, then there exists a \(v \in R\) such that \(vu = 1\) and we have
    \[
...
Ring homomorphisms also preserve units: that is, if \(u\) is a 

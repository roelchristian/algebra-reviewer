...
\begin{example}
    ...
    \id_A = f = \id_B \circ f\). Thus \(\catg{Set}\) is a category by our
    definition.
\end{example}
...
\begin{example}[Coslice category]
    ...
    category \(\catg{Set}\) and the singleton \(\{*\}\) which is an object of
    this category. We will call the resulting category \(\catg{Set_*}\).

    The objects of \(\catg{Set_*}\) will then be morphisms in \(\catg{Set}\) of
    the form \(f: \{*\} \to X\) for some set \(X\). Each morphism (in
    \(\catg{Set}\)) of this type fixes an element \(x \in X\); i.e., \(f\)
    determines a unique element \(x \in X\) and \(x\) determines \(f\). Thus we
    can denote the objects of \(\catg{Set_*}\) to be the elements of the ordered
    pairs \((X, x)\).
...
\begin{example}
    Given two objects \(A\) and \(B\) of a category \(\catg{C}\) we know construct a new category \(\catg{C}_{A, B}\) whose objects are diagrams of \(\catg{C}\) of the form
    \[
        \begin{tikzcd}
                    & A \\
...
denote this inverse by \(f^{-1}\) without ambiguity. The following properties of isomorphisms are immediate from the definition:

\begin{theorem}
    Let \(\catg{C}\) be a category.
    \begin{enumerate}[label=(\alph*), itemsep=0pt]
        \item For each object \(A\) in \(\catg{C}\), the identity morphism \(\id_A\) is an isomorphism and is its own inverse.
        \item If \(f: A \to B\) is an isomorphism, then its inverse \(f^{-1}\) is also an isomorphism and \((f^{-1})^{-1} = f\).
        \item If \(f : A \to B\) and \(g : B \to C\) are isomorphisms, then so is \(g \circ f\) and \((g \circ f)^{-1} = f^{-1} \circ g^{-1}\).
    \end{enumerate}
\end{theorem}
...
the isomorphisms in the category \(\catg{Set}\). In the category \(\catg{R}\)
...
considering the category \(\catg{R}\) obtained from a set \(S\) and a relation
\(\sim\) on \(S\) that is reflexive and transitive (we have constructed this
category in Example~\ref{ex:category-from-set-relation}). Now suppose that
...
Having earlier characterized monomorphisms and epimorphisms in \(\catg{Set}\) in
...
reference to objects of \(\catg{Set}\) with a more general reference to objects
of an arbitrary category \(\catg{C}\).
...
In Theorem~\ref{thm:injective-surjective-monomorphism-epimorphism}, we have already shown that monomorphisms and epimorphisms in \(\catg{Set}\) are precisely the injective and surjective functions, respectively. In Example~\ref{ex:category-from-set-relation}